...
import { attributesToFindOptions, getQueries } from "./query"
...
export default ({
  ...
  result.queries = getQueries({model, modelTypes, schemaConfig})

  result.mutations = getMutations({model, modelTypes})

  result.subscriptions = getSubscriptions({model, modelTypes})
